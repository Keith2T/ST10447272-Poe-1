...
public String getLongestTaskDetails() {
    int maxDuration = 0;
    int maxIndex = 0;
    for (int i = 0; i < taskDurations.size(); i++) {
        if (taskDurations.get(i) > maxDuration) {
    ...
    return developers.get(maxIndex) + ", " + maxDuration;
}

public String searchTaskByNameForTest(String name) {
    for (int i = 0; i < taskNames.size(); i++) {
        if (taskNames.get(i).equals(name)) {
...
public String[] searchTasksByDeveloperForTest(String developer) {
...
public boolean deleteTaskForTest(String taskName) {
    for (int i = 0; i < taskNames.size(); i++) {
        if (taskNames.get(i).equals(taskName)) {
            taskNames.remove(i);
            developers.remove(i);
